...
class Account {
    ...
    private async startSync(daysWithin : number) : Promise<SyncResponse>{
        const response = await axios.post<SyncResponse>(`${API_BASE_URL}/email/sync`,
            {},
            {
                headers : {
    ...
    async syncEmails(){
        ...
        if(!account){
            throw new Error("Account not found invalid token")
        }
        ...
        try {
            ...
            console.log("error in account.ts" , error);
        }
    }
